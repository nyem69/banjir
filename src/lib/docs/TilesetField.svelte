<script lang="ts">
  ...
  export let doubleScale = browser && window?.devicePixelRatio > 1 || false;

  import { onMount } from 'svelte';
  onMount(() => {
    doubleScale = window.devicePixelRatio > 1;
  });    
  ...
  const ACCESS_TOKEN = 'pk.eyJ1IjoibnllbSIsImEiOiJBZGdTcWYwIn0.Qodnwy5WNQVNB4Khe0Au-g';
